...
int parser(int argc, char **argv, list_flag *flag, char **file_list,
           int *len_file_list) {
  init(flag);
  int err = 0;
  if (check_file(argc, argv, file_list, len_file_list) &&
...
#ifdef DEBAG
  for (int i = 0; i < *len_file_list; ++i) printf("%s \n", file_list[i]);
#endif
...
void write_file(FILE *file, list_flag *flag, int reset) {
  ...
  while (feof(file) != EOF && file) {
    int ch = getc(file);
    if (ch == EOF) break;
    if (remove_blank_lines(ch, &c, flag)) continue;
    c.prev_prev = c.prev;
    c.prev = ch;
